import create_op_rate_graph as corg
import statistics as stat
from scipy import stats as scipystat


def analyze_topology_results(operators, instances, duration, repetitions, stats_folder, id_prefix):
    generate_individual_files = False

    start_ts = duration * 0.1
    end_ts = duration * 0.9

    reps_throughput = {}
    reps_latency = {}
    reps_cost = {}
    reps_invocations = {}
    for o in operators:
        reps_throughput[o] = []
        reps_latency[o] = []
        reps_cost[o] = []
        reps_invocations[o] = []

    for r in range(0, repetitions):

        # compute experiment id
        id = str(r) + '_'
        for o in operators:
            id += str(instances[o]) + '_'
        id += id_prefix + '_'

        data = corg.manage_topology_parallel_op_graphs(stats_folder,
                                                       [id + op for op in operators],
                                                       list(instances.values()),
                                                       start_ts, end_ts)
        for o in operators:
            reps_throughput[o].append(scipystat.trim_mean(data[id + o + "_rate"][1][start_ts:end_ts], 0.05))
            if id + o + "_latency" in data:
                reps_latency[o].append(scipystat.trim_mean(data[id + o + "_latency"][1][start_ts:end_ts], 0.05))
            reps_invocations[o].append(scipystat.trim_mean(data[id + o + "_invocations"][1][start_ts:end_ts], 0.05))
            reps_cost[o].append(scipystat.trim_mean(data[id + o + "_cost"][1][start_ts:end_ts], 0.05))

    throughput = {}
    latency = {}
    cost = {}
    invocations = {}
    for o in operators:

        throughput[o] = stat.mean(reps_throughput[o])
        if len(reps_latency[o]) > 0:
            latency[o] = stat.mean(reps_latency[o])
        else:
            latency[o] = 0.0
        cost[o] = stat.mean(reps_cost[o])
        invocations[o] = stat.mean(reps_invocations[o])

        print(o + ': T ' + str(throughput[o]) + ' L ' + str(latency[o])
              + ' C ' + str(cost[o] * invocations[o] / instances[o] / pow(10, 9)))

    return [list(throughput.values()), list(latency.values()), list(cost.values()), list(invocations.values())]
