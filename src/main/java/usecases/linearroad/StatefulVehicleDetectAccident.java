...
public class StatefulVehicleDetectAccident {
	...
	public static void main(String[] args) throws AlreadyAliveException,
		...
		class CheckNewSegment implements BoltFunction {
			...
			private DetectAccidentOperator detectAccOperator;
			...
			public List<Values> process(Tuple arg0) {
				...
				if (arg0.getIntegerByField("lr_type") == 0) {
					...
					UpdateSegmentAnswer result = detectAccOperator.run(
							...
							arg0.getIntegerByField("lr_speed"),
			...
			public void prepare(Map arg0, TopologyContext arg1) {
				detectAccOperator = new DetectAccidentOperator();
			}
		...
		BoltDeclarer op = builder.setBolt("op", new ViperBolt(new Fields(
				...
				"lr_lane", "lr_dir", "lr_seg", "lr_pos", "new_acc",
				"cleared_acc"), new CheckNewSegment()), op_parallelism);
		...
		if (useOptimizedQueues) {
			...
			if (op_parallelism == 1) {
				...
				builder.setBolt(
						...
						new ViperMerger(new Fields("lr_type", "lr_time",
								"lr_vid", "lr_speed", "lr_xway", "lr_lane",
								"lr_dir", "lr_seg", "lr_pos", "new_acc",
								"cleared_acc"), "lr_time"), sink_parallelism)
						.shuffleGrouping("op");
